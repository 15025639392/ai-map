...
```json
{
  ...
  "story_id": "S-005-1",
  "epic_id": "E-005",
  "task_goal": "完成分布式部署（网关+核心服务+缓存+存储）",
  "owner_role": "devops_engineer",
  "development_executor": "devops_engineer_lead",
  "estimate_days": 10,
  "acceptance_criteria": [
...
```text
你现在扮演 devops_engineer_lead，负责实现任务 T-005-2。

【任务信息】
- epic/story: E-005/S-005-1
- owner_role: devops_engineer
- estimate_days: 10
- task_goal: 完成分布式部署（网关+核心服务+缓存+存储）

【边界与约束】
- 允许修改：infra/**, deploy/**, scripts/**, ci/**
- 禁止修改：src/index.ts
...
```text
你现在扮演 Reviewer，对任务 T-005-2 做发布前审查。
...
```text
你现在扮演 QA 验收工程师，仅根据验收标准判断任务 T-005-2 是否通过。
