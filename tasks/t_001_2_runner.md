...
```json
{
  ...
  "story_id": "S-001-1",
  "epic_id": "E-001",
  "task_goal": "实现 Layer 基类生命周期（add/remove/show/hide）",
  "owner_role": "frontend_engineer",
  "development_executor": "frontend_engineer_lead",
  "estimate_days": 6,
  "acceptance_criteria": [
...
```text
你现在扮演 frontend_engineer_lead，负责实现任务 T-001-2。
...
```text
你现在扮演 Reviewer，对任务 T-001-2 做发布前审查。
...
```text
你现在扮演 QA 验收工程师，仅根据验收标准判断任务 T-001-2 是否通过。
