...
export class EditController {
    layers = [];
    undoRedoManager;
    currentTool = EditToolType.NONE;
    currentState = EditState.IDLE;
    drawingPoints = [];
    selection = { features: [], vertices: [] };
    config;
    constructor(config = {}) {
        this.config = {
     ...
     * 激活绘制面工具
     */
    activateDrawPolygonTool(options) {
     ...
     * 执行点绘制
     */
    async executeDrawPoint(position) {
     ...
     * 执行线绘制
     */
    async executeDrawLine(coordinates) {
        const features = this.getAllFeatures();
     ...
     * 执行面绘制
     */
    async executeDrawPolygon(rings) {
        const features = this.getAllFeatures();
     ...
     * 移动顶点
     */
    moveVertex(vertexInfo, newPosition) {
        const features = this.getAllFeatures();
