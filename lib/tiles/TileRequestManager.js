...
export class TileRequestManager {
        ...
        while (tile.retryCount <= this._config.maxRetries) {
            ...
            try {
                ...
                if (tile.retryCount <= this._config.maxRetries) {
                    // 计算重试延迟（指数退避）
                    const delay = Math.min(this._config.retryDelayBase * Math.pow(2, tile.retryCount - 1), this._config.retryDelayMax);
     ...
     * 带超时的加载
     */
    async _loadWithTimeout(tile) {
        return Promise.race([
            this._config.loadFn(tile),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Tile request timeout')), this._config.requestTimeout)),
        ]);
    }
    /**
     * 等待并发限制
     */
    async _waitForConcurrency() {
...
//# sourceMappingURL=TileRequestManager.js.map