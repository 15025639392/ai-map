 ...
 * 瓦片 ID 生成器
 */
function generateTileId(coord: ITileCoord): string {
...
export class TileQueue {
  private _config: Required<ITileQueueConfig>;
  private _requestManager: TileRequestManager;
  private _tiles: Map<string, ITile> = new Map();
  private _pendingTiles: Set<string> = new Set();
  private _eventListeners: Map<TileQueueEvent, Set<TileQueueEventListener>> = new Map();
   ...
   * 添加瓦片到队列
   */
  addTile(coord: ITileCoord, url: string, priority: number = 0): ITile {
   ...
   * 批量添加瓦片
   */
  addTiles(tiles: Array<{ coord: ITileCoord; url: string; priority?: number }>): ITile[] {
  ...
  getPendingTiles(): ITile[] {
    return Array.from(this._pendingTiles)
      ...
      .filter((tile) => tile !== undefined) as ITile[];
  }
  ...
  getLoadingTiles(): ITile[] {
    return Array.from(this._tiles.values()).filter(
      (tile) => tile.state === TileState.LOADING
    );
  }
  ...
  getLoadedTiles(): ITile[] {
    return Array.from(this._tiles.values()).filter(
      (tile) => tile.state === TileState.LOADED
    );
  }
  ...
  getFailedTiles(): ITile[] {
    return Array.from(this._tiles.values()).filter(
      (tile) => tile.state === TileState.FAILED
    );
  }
   ...
   * 启动处理队列
   */
  private async _startProcessing(): Promise<void> {
   ...
   * 获取待处理的瓦片列表
   */
  private _getTilesToProcess(): ITile[] {
   ...
   * 处理单个瓦片
   */
  private async _processTile(tile: ITile): Promise<void> {
    // 移除待处理标记
   ...
   * 触发事件
   */
  private _emit(event: TileQueueEvent, tile: ITile): void {
    const listeners = this._eventListeners.get(event);
    if (listeners) {
