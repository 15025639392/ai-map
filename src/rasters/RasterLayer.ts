...
export class RasterLayer extends Layer {
   ...
   * 加载可见瓦片
   */
  private _loadVisibleTiles(): void {
   ...
   * 加载单个瓦片
   */
  private _loadTile(x: number, y: number, z: number): void {
    const tileKey = `${z}-${x}-${y}`;
    const tile: ITile = {
      x,
      y,
   ...
   * 瓦片加载完成回调
   */
  private _onTileLoad(tileKey: string, img: HTMLImageElement): void {
    const gl = this._gl;
    if (!gl) return;
  ...
  add(renderer: IRenderer): void {
    super.add(renderer);
    this._gl = renderer.gl as WebGL2RenderingContext;
    this._initShaders();
    this._loadVisibleTiles();
  }
